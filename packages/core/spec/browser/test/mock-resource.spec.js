...
import { headerMiddleware, headerMiddlewareV2, asyncHeaderMiddleware } from '../../helper'
...
describe('Test lib / mock resources', () => {
  ...
  it('matches the body params independent of order', (done) => {
    ...
    client.Blog.post({
      ...
      .catch((response) => {
        const error = response.rawData ? response.rawData() : response
        done(`test 1 failed with promise error: ${error}`)
      })
    ...
    client.Blog.post({
      ...
      .catch((response) => {
        const error = response.rawData ? response.rawData() : response
        done(`test 2 failed with promise error: ${error}`)
      })
  })
