...
export const defaultRetryConfigs: Required<RetryMiddlewareOptions> = {
 ...
 */
export const RetryMiddleware = (customConfigs: RetryMiddlewareOptions = {}): Middleware =>
  function RetryMiddleware() {
    return {
      response(next, _renewFn, request) {
        const retryConfigs = assign({}, defaultRetryConfigs, customConfigs)
        const enableRetry = request.method() === 'get'
...
type RetryFn = (
  retryTime: number,
  retryCount: number,
  retryConfigs: Required<RetryMiddlewareOptions>
) => void
...
export const calculateExponentialRetryTime = (
  retryTime: number,
  retryConfigs: Required<
    Pick<RetryMiddlewareOptions, 'factor' | 'multiplier' | 'maxRetryTimeInSecs'>
  >
) =>
  Math.min(
