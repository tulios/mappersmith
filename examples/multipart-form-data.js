...
const client = forge({
  host: 'https://host.url',
  middlewares: [
    BasicAuthMiddleware({ username: 'meowmeow', password: 'beenz' }),
  ],
  resources: {
    File: {
...
formData.append('fileFromBuffer', Bufer.from('file contents'), 'filename.ext')

//Append files as part of a file array
formData.append('files[]', fs.createReadStream('path-to-file'), 'filename.ext')
formData.append('files[]', Bufer.from('file contents'), 'filename.ext')
